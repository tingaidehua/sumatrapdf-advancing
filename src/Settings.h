...
struct GlobalPrefs {
    ...
    bool showFavorites;
    // if true, show the Library sidebar
    bool showLibrary;
    // width of the Library sidebar in screen pixels at 96 DPI; values
    // below 160 use the default
    int libraryDx;
    ...
    // bookmarks always keep the reader's current zoom. If true,
    // destination links inside the document also keep the current zoom
    // instead of applying the zoom the destination asks for; links still
    // go to the target page and position. Same as Adobe Reader's 'forbid
    // the change of the current zoom factor during execution of Go to
    // Destination actions'
...
static const FieldInfo gGlobalPrefsFields[] = {
    ...
    {offsetof(GlobalPrefs, libraryDx), SettingType::Int, 280},
...
static const StructInfo gGlobalPrefsInfo = {
    ...
    "SearchUIFloating\0ShowFavorites\0ShowLibrary\0LibraryDx\0SortFavoritesByName\0ShowToc\0SidebarOnRight\0ShowLinks\0"
    "HighlightFormFields\0ClickEdgeToTurnPage\0DisableLinks\0RememberViewOffsetOnPageTurn\0MouseWheelTurnsPage\0ShowDoc"
    "umentFocusIndicator\0ShowAnnotationNotification\0ShowTocPageNumbers\0ShowStartPage\0SidebarDx\0Scrollbars\0Scrollb"
    "arInSinglePage\0SmoothScroll\0ScrollLineAmount\0PaddingAfterLastPage\0IgnoreDestinationZoom\0HighlightLinkDestinat"
    "ion\0CitationHoverDelay\0ReadAloudVoiceId\0ReadAloudSpeed\0FastScrollOverScrollbar\0PreventSleepInFullscreen\0TabW"
    "idth\0Theme\0LastLightTheme\0LastDarkTheme\0DocumentColorsFollowTheme\0TocDy\0ToolbarCustomLayout\0ToolbarShowRead"
    "Aloud\0ToolbarSize\0TreeFontName\0TreeFontSize\0UIFontSize\0DisableAntiAlias\0EngineeringDrawingEnhance\0DisableAu"
    "toLinks\0UseSysColors\0UseTabs\0SelectionToolbar\0TabsMru\0CtrlTabSimple\0ZoomLevels\0ZoomIncrement\0\0FixedPageUI"
    ...
    "instead of the compact toolbar overlay\0if true, show the Favorites sidebar\0if true, show the Library "
    "sidebar\0width of the Library sidebar in screen pixels at 96 DPI; values below 160 use the default\0if true, "
    ...
    "languages already put it on the right)\0if true, draw a blue border around links in the document\0if true, "
    ...
    "link follows it\0if true, next/previous page keeps the same view position on the page instead of jumping to the "
    "top (useful when zoomed in on similarly sized pages)\0if true, one mouse-wheel notch goes to the next / previous "
    "page instead of scrolling; combine with RememberViewOffsetOnPageTurn to read zoomed-in pages without touching the "
    ...
    "the document to the top of the window\0bookmarks always keep the reader's current zoom. If true, destination "
    "links inside the document also keep the current zoom instead of applying the zoom the destination asks for; links "
    "still go to the target page and position. Same as Adobe Reader's 'forbid the change of the current zoom factor "
    "during execution of Go to Destination actions'\0if true, following an internal link or bookmark flashes a "
